...
parser.add_argument("dim", metavar="dim", type=int, default=2, help="Dimension of the problem")
parser.add_argument(
    "run",
    metavar="n_runs",
    type=int,
...
bounds_file = Path(
    ...
    "data",
    "simulated_data",
    f"{dim}dim_benchmark_f{func_number}_bounds.csv",
)
...
output_dir = Path(results_dir, f"{dim}D_f{func_number}", f"{dim}D_f{func_number}_run{run}")

art_params = {
    "bounds": utils.read_table(bounds_file),
...
if init_cycle == 1:
    ...
    mae_score_train = np.genfromtxt(str(Path(output_dir, "mae_score_train.csv")), delimiter=",")
    mae_score_test = np.genfromtxt(str(Path(output_dir, "mae_score_test.csv")), delimiter=",")
    best_prediction = np.genfromtxt(str(Path(output_dir, "best_prediction.csv")), delimiter=",")
    std = np.genfromtxt(str(Path(output_dir, "std.csv")), delimiter=",")
    prob_success = np.genfromtxt(str(Path(output_dir, "prob_success.csv")), delimiter=",")
...
for cycle in range(init_cycle - 1, n_cycles):
    ...
    art = RecommendationEngine(df=df, result_suffix=f"_Cycle{cycle+1}", **art_params)
    ...
    # Evaluate models
    art.evaluate_models()
    mae_score_train[cycle] = art.model_prediction_dfs[0]["MAE"]["Ensemble Model"]
    X_test = art.recommendations.values[:, :-1]
    y_test = func(X_test.T, dim).reshape(-1, len(art.response_vars))
    art.evaluate_models(X_test, y_test)
    mae_score_test[cycle] = art.model_prediction_dfs[0]["MAE"]["Ensemble Model"]
    ...
    if run == 1:
        prior_suffix = art.result_suffix
        art.result_suffix = f"_Cycle{cycle}"
        ...
        art.result_suffix = prior_suffix
